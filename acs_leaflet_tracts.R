...
##
t_NE = tracts(state='NE', cb=TRUE)
t_KS = tracts(state='KS', cb=TRUE)
t_IA = tracts(state='IA', cb=TRUE)
t_MO = tracts(state='MO', cb=TRUE)
tracts_shapes <- rbind_tigris(t_NE, t_KS, t_IA, t_MO)


##
## 3) Get the tabular data (acs) 
##     
## make geo of all the tracts in the four states
NeKsIaMo_tracts <- geo.make(state=c('NE','KS','IA','MO'), county = '*', tract = '*', check = TRUE)
# TODO: make geo of all the 15 counties in the Service Area

# get total population:
total_pop <- acs.fetch(geo=NeKsIaMo_tracts, 
...
total_pop_df <- data.frame(paste0(str_pad(total_pop@geography$state, 2, "left", pad="0"), 
                                  ...
                                  str_pad(total_pop@geography$tract, 6, "left", pad="0")
                                  ),
...
## 
total_pop_merged<- geo_join(tracts_shapes, total_pop_df, "GEOID", "GEOID")
...
enrl_level <- acs.fetch(geo=NeKsIaMo_tracts, 
...
enrl_level_df <- enrl_level_df[enrl_level_df$Total>0,]
...
enrl_level_df$High.School.percent[is.nan(enrl_level_df$High.School.percent)] <- 0
enrl_level_df$Undergraduate.percent[is.nan(enrl_level_df$Undergraduate.percent)] <- 0
enrl_level_df$Grad.Professional.percent[is.nan(enrl_level_df$Grad.Professional.percent)] <- 0

## 
## 4) Do the merge (tigris)
## 
enrl_level_merged<- geo_join(tracts_shapes, enrl_level_df, "GEOID", "GEOID")
# there are some tracts with no land that we should exclude
